...
///* m_copy2 -- copies matrix into new area
//	-- out(i0:m,j0:n) <- in(i0:m,j0:n) */
//#ifndef ANSI_C
//MAT2 m_copy2(in)
//MAT2 in;
//#else
//MAT2 m_copy2(const MAT2 in)
//#endif
//{
//  MAT2 out;
...
/* m_free2 -- returns MAT & associated memory back to memory heap */
#ifndef ANSI_C
int	m_free2(mat)
MAT2 *mat;
#else
int	m_free2(MAT2 *mat)
#endif
{
#ifdef SEGMENTED
  int i;
...
/* v_free2 -- returns VEC & associated memory back to memory heap */
#ifndef ANSI_C
int	v_free2(vec)
VEC2 *vec;
#else
int	v_free2(VEC2 *vec)
#endif
{
  if( vec==(VEC2 *)NULL || (int)(vec->dim) < 0 )
    /* don't trust it */
    return (-1);
...
/* _m_pow2 -- computes integer powers of a square matrix A, A^p
   -- uses tmp as temporary workspace */
#ifndef ANSI_C
MAT2 _m_pow2(A, p, tmp)
MAT2 A, tmp;
int	p;
#else
...
//    	printf("it_cnt=%d\n", it_cnt);
//        m_mlt(Z(it_cnt), Z(it_cnt), Z(it_cnt+1));
//    	aux = (((it_cnt+1) & 1) ? tmp : out);
//    	aux2 = (((it_cnt) & 1) ? tmp : out);
//    	aux = m_mlt2(aux2, aux2);
//    	printf("aux2=%f\n", aux2.me[0][0]);
...
//        printf("*Z(it_cnt+1)=%f\n", Z(it_cnt+1)->me[0][0]);
        ...
        if(p & (1 << (max_bit-1)))
        {
//          MAT2 test = m_mlt2(A, Z(it_cnt));
//          Z(it_cnt+1) = m_copy2(test);
//          aux = m_mlt2(A, aux2);
//          printf("aux22=%f\n", aux2.me[0][0]);
...
/* m_pow2 -- computes integer powers of a square matrix A, A^p */
#ifndef ANSI_C
MAT2 m_pow2(A, p)
MAT2 A;
int	p;
#else
MAT2 m_pow2(const MAT2 A, int p)
#endif
{
  MAT2 out;
...
#endif
{
  ...
  limit = A.m - 1;
  printf("hfactor2_test\n");
  hh = v_get2(A.m);
  printf("hfactor2_test2\n");
  w  = v_get2(A.n);
  printf("hfactor2_test3\n");

  for(k = 0; k < limit; k++)
  {
    /* compute the Householder vector hh */
	hh = get_col2(A, (unsigned int)k);
	printf("hfactor2_test4\n");
	hh = hhvec2(hh, k+1, &beta.ve[k], &A.me[k+1][k]);
	printf("hfactor2_test5\n");
    diag.ve[k] = (((k+1)>=0 && (k+1)<(hh).dim) ? (hh).ve[(k+1)] : \
        0);
    printf("hfactor2_test6\n");
//    v_set_val2(diag, k, v_entry2(hh, k+1));

    /* apply Householder operation symmetrically to A */
    b = v_entry2(beta, k);
    printf("hfactor2_test7\n");
    A = _hhtrcols2(A, k+1, k+1, hh, b, w);
    printf("hfactor2_test8\n");
    A = hhtrrows2(A, 0, k+1, hh, b);
    printf("hfactor2_test9\n");
  }
  printf("here\n");

//#ifdef THREADSAFE
//  v_free2(&hh);  v_free2(&w);
//#endif
  printf("hfactor2_test10\n");
  return (A);
}
